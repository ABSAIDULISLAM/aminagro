...
class AnimalStatisticsController extends Controller
{
	...
	public function getAnimalDetails(Request $request){
		if($request->id){			
            ...
            $data['additional_expense'] = DB::table('permanent_expense')
                ->where('cow', $request->id)
                ->where('status', 0)
                ->select('id', 'name', 'date', 'description', 'price') // Explicitly select needed fields
                ->orderBy('date', 'desc')
                ->get();
            
            $data['additional_expense_sum'] = DB::table('permanent_expense')
                ->where('cow', $request->id)
                ->where('status', 0)
                ->sum('price');
                
            $data['milk_collection'] = DB::table('collect_milk')
                ->where('dairy_number', $request->id)
                ->orderBy('date', 'desc')
                ->get();

            $data['milk_collection_sum'] = DB::table('collect_milk')
                ->where('dairy_number', $request->id)
                ->sum('total');
            
            
$data['food'] = DB::table('cow_feed')
    ->join('cow_feed_dtls', 'cow_feed_dtls.feed_id', '=', 'cow_feed.id')
    ->join('food_items', 'cow_feed_dtls.item_id', '=', 'food_items.id')
    ->join('food_units', 'cow_feed_dtls.unit_id', '=', 'food_units.id')
    ->where('cow_feed.cow_id', $request->id)
    ->select(
        'cow_feed.*',
        'cow_feed_dtls.*',
        'food_items.name as food_name',
        'food_items.id as food_items_id',
        'cow_feed_dtls.qty as cow_feed_dtls_qty',
        'food_units.name as cow_feed_dtls_unit',
        'cow_feed.date as feeding_date'
    )
    ->orderBy('cow_feed.date', 'desc')
    ->get();

// Get all food prices in one query
$foodItems = $data['food']->pluck('food_items_id')->unique();
$prices = DB::table('cow_food')
    ->whereIn('food', $foodItems)
    ->orderBy('id', 'desc')
    ->get()
    ->groupBy('food');

// Calculate prices
$data['totalPriceAllRecords'] = 0;

foreach ($data['food'] as $item) {
    if (isset($prices[$item->food_items_id])) {
        $latestPrice = $prices[$item->food_items_id]->first();
        $item->price_per_unit = $latestPrice->price;
        $item->total_price = $latestPrice->price * $item->cow_feed_dtls_qty;
        $data['totalPriceAllRecords'] += $item->total_price;
    } else {
        ...
        $item->total_price = null;
            ...
            $data['cow_sell_price'] = DB::table('cow_sale_dtls')
		...
		public function getAnimalDetails_leg(Request $request){
		if($request->id){			
    ...
    if (isset($prices[$item->food_items_id])) {
        ...
        $item->total_price = null;
            ...
            $data['cow_sell_price'] = DB::table('cow_sale_dtls')
                ->where('cow_id', $request->id)
                ->first();    
			return view('animal-statistics.ajax-animal-leg', $data);
		} else {
			return '';
		}
	}
