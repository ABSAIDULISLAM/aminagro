...
class AnimalPregnancyController extends Controller
{
    ...
    Expense::create([
        'purpose_id' => 8,
        'date' => $date,
        'amount' => $request->semen_cost + $request->other_cost,
        'note' => $request->note,
        'created_by' => 1,
    ]);
	...
	public function animal_pregnancy_list(){
	    $pregnancylist = DB::table('pregnancy_record')
    ->join('pregnancy_type', 'pregnancy_record.pregnancy_type_id', '=', 'pregnancy_type.id')
    ->join('sheds', 'pregnancy_record.stall_no', '=', 'sheds.id')
    ->join('animals', 'pregnancy_record.cow_id', '=', 'animals.id')
    ->join('animal_type', 'pregnancy_record.semen_type', '=', 'animal_type.id')
    ->select(
        'pregnancy_record.*',
        'pregnancy_type.type_name as pregnancy_type_name',
        'animal_type.type_name as animal_type_name',
        'sheds.shed_number as shed_name'
    )
    ->get();
	    return view('animal-pregnancy.animal-pregnancy-list', compact('pregnancylist'));
	}
}
