...
class DashboardController extends Controller
{
	public function index() {
		...
		$data['PermanentExpense'] = PermanentExpense::sum('price');
        ...
        // Calculate totals
        $data['totalEarnings'] = $ledger->where('type', 'Income')->sum('amount');
        $data['totalExpenses'] = $ledger->where('type', 'expense')->sum('amount');
        $today = now()->format('Y-m-d'); // Get today's date in 'Y-m-d' format

        $data['TodysEarnings'] = $ledger->where('type', 'Income')
                                    ->where('date', $today)
                                    ->sum('amount');
        $data['TodysExpenses'] = $ledger->where('type', 'expense')
                                    ->where('date', $today)
                                    ->sum('amount');
		...
		$data['total_sheds'] = Shed::where('branch_id', Session::get('branch_id'))->count();
