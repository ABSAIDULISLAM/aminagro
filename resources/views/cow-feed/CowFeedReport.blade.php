...
<button onclick="window.print()" class="btn btn-primary">Print <i class="fa-solid fa-print"></i> </button>
  ...
  <tbody>
    ...
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $cow_foods->branch_name }}</td>
      <td>{{ $cow_foods->supplier }}</td>
      <td>{{ $cow_foods->food_name }}</td>
      <td>{{ $cow_foods->quantity }} {{ $cow_foods->unit }}</td>
      <td>{{ $cow_foods->price }}</td>
      <td>{{ $cow_foods->price * $cow_foods->quantity }}</td>
      <td>{{ $cow_foods->description }}</td>
      <td>{{ $cow_foods->date }}</td>
      <td>
        <!-- Edit Button -->
        <a href="{{ route('cow_food.edit', $cow_foods->id) }}" class="btn btn-sm btn-primary">
        <i class="fa fa-edit"></i> Edit
        </a>
        <!-- Delete Button -->
        <form action="{{ route('cow_food.delete', $cow_foods->id) }}" method="POST" style="display:inline;">
        @csrf
        @method('DELETE')
