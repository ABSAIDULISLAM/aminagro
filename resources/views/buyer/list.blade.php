...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    @foreach($buyer as $index => $buyers)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $buyers->name }}</td>
      <td>{{ $buyers->phone }}</td>
      <td>{{ $buyers->address }}</td>
      <td>{{ $buyers->balance }}</td>
      <td><a href="{{ route('buyer.edit', $buyers->id) }}" class="btn btn-sm btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>                  <form action="{{ route('buyer.destroy', $buyers->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this buyer?')">
