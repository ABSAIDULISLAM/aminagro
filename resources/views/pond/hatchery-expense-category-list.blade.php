...
@section('title', __('খরচ ক্যাটেগরী তালিকা'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="icon-list"></i> {{__('খরচ ক্যাটেগরী তালিকা') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('খরচ ক্যাটেগরী তালিকা') }}</li>
  </ol>
</section>
...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    @foreach($HatcheryExpenseCategory as $index => $HatcheryExpenseCategorys)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $HatcheryExpenseCategorys->name }}</td>
    </tr>
    @endforeach
  </tbody>
</table>
