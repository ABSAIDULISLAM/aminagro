      ...
      <div class="col-md-8 col-md-offset-2" style="margin-left:0px; width: 100%;">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <form action="{{ route('fish_stocking_report_by_date') }}" method="GET">
                     @csrf
                <div class="col-md-6">
...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    @foreach($FishStocking as $index => $FishStockings)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $FishStockings->pond_name }}</td>
      <td>{{ $FishStockings->fish_name }} - {{ $FishStockings->id }}</td>
      <td>{{ $FishStockings->qty }}</td>
      <td>{{ $FishStockings->price }}</td>
      <td>{{ $FishStockings->date }}</td>
      <td>        <a href="{{ route('fish_stocking.edit', $FishStockings->id) }}" class="btn btn-warning btn-sm">
          <i class="fa-solid fa-pen-to-square"></i> Edit
        </a>         
        <form action="{{ route('fish_stocking.destroy', $FishStockings->id) }}" method="POST" style="display:inline;">
            @csrf
            @method('DELETE')
