...
@section('title', __('Fish Selling Report'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="icon-list"></i> {{__('Fish Selling Report') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('Fish Selling Report') }}</li>
  </ol>
</section>
      ...
      <div class="col-md-8 col-md-offset-2" style="margin-left:0px; width: 100%;">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <form action="{{ route('fish_sell_report_by_date') }}" method="GET">
                     @csrf
                <div class="col-md-6">
...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    @foreach($FishSell as $index => $FishSells)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $FishSells->id }}</td>
      <td>{{ $FishSells->pond_name }}</td>
      <td>{{ $FishSells->buyer_name }}</td>
      <td>{{ $FishSells->fish_name }}</td>
      <td>{{ $FishSells->qty }} {{ $FishSells->unit }}</td>
      <td>{{ $FishSells->price }}</td>
      <td>{{ $FishSells->total_price }}</td>
      <td>{{ $FishSells->paid_amount }}</td>
      <td>{{ $FishSells->total_price - $FishSells->paid_amount }}</td>
      <td>{{ $FishSells->date }}</td>
      <td> 
        <a href="{{ route('fish_sell.edit', $FishSells->id) }}" class="btn btn-warning btn-sm">
        <i class="fa-solid fa-pen-to-square"></i> Edit
        </a>
        
        <form action="{{ route('fish_sell.destroy', $FishSells->id) }}" method="POST" style="display:inline;">
        @csrf
        @method('DELETE')
