...
<form action="{{ route('fish_sell.update', $FishSell->id) }}" method="POST">
    @csrf
  <div class="form-row">
        ...
        <option value="{{ $pond->id }}" {{ $FishSell->pond == $pond->id ? 'selected' : '' }}>
        {{ $pond->name }}
        </option>
        @endforeach
        </select>
        ...
        <option value="{{ $buyer->id }}" {{ $FishSell->buyer == $buyer->id ? 'selected' : '' }}>
        {{ $buyer->name }}
        </option>
        @endforeach
        </select>
        ...
        <input type="text" class="form-control" id="inputEmail4" placeholder="মাছের নাম" name="fish_name" value="{{ $FishSell->fish_name }}">
        </div>
        
        <div class="form-group col-md-12">
        <label for="inputEmail4"> মাছের পরিমাণ*</label>
        <input type="text" class="form-control" id="inputEmail4" placeholder="মাছের পরিমাণ" name="qty" value="{{ $FishSell->qty }}">
        </div>
        
        <div class="form-group col-md-12">
        <label for="inputEmail4">ইউনিট প্রতি মূল্য*</label>
        <input type="text" class="form-control" id="inputEmail4" placeholder="ইউনিট প্রতি মূল্য" name="price" value="{{ $FishSell->price }}">
        </div>
        
        <div class="form-group col-md-12">
        <label for="inputEmail4">প্রদানকৃত মূল্যের পরিমান*</label>
        <input type="text" class="form-control" id="inputEmail4" placeholder="প্রদানকৃত মূল্যের পরিমান" name="paid_amount" value="{{ $FishSell->paid_amount }} "  >
        </div>
        
        <div class="form-group col-md-12">
        <label for="inputEmail4">তারিখ*</label>
        <input type="date" class="form-control" id="inputEmail4" placeholder="তারিখ" name="date" value="{{ $FishSell->date }}" style="width: 25%;">
        </div>
  </div>
