...
@section('title', __('Fish Sell Due Collection'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="icon-list"></i> {{__('Fish Sell Due Collection') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
    <li class="active"><a>{{__('Fish Sell Due Collection') }}</a></li>
  </ol>
</section>
<!-- Main content -->
<section class="content">
  ...
  <div class="box box-success">
    <div class="box-body">
      <div class="form-group"> {!! Form::open(array('url' => 'getFishSellHistory','class'=>'form-horizontal','autocomplete'=>'off','method' =>'GET')) !!}
      ...
      <div class="col-md-12">
        <table class="table table-responsive table-striped table-bordered">
          ...
          <tr>
            ...
            <td>000{{$data->id}}</td>
            <td>{{App\Library\farm::currency($data->total_price)}}</td>
          ...
          <tr>
            ...
            <td class="total-paid"><b>{{$data->paid_amount}}</b></td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td colspan="3" align="right"><b>{{__('sale_milk.total_amount') }} :</b></td>
            <td class="total-amount"><b>{{App\Library\farm::currency($data->total_price)}}</b></td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td colspan="3" align="right"><b>{{__('sale_milk.due') }} :</b></td>
            <td class="due-amount"><b>{{App\Library\farm::currency($allData[0]->total_price - $data->paid_amount )}}</b></td>
            <td>&nbsp;</td>
          </tr>
          <tr>
            <td colspan="5" align="right"><a target="_blank" href="{{URL::to('sale-fish-invoice')}}/{{$data->id}}" class="btn btn-default btn-sm"><b> <i class="icon-doc"></i> {{__('sale_milk.view_invoice') }}</b></a> @if($allData[0]->total_amount > $totalPaid) <a href="#paymentModal" data-toggle="modal" class="btn btn-success btn-sm"> <i class="fa fa-money"></i><b> {{__('sale_milk.pay_due') }}</b></a> @endif </td>
          </tr>
        <tr>
            <td colspan="5" align="right"><a href="{{URL::to('sale-fish-update-pay')}}/{{$data->id}}" class="btn btn-success btn-sm"><b> <i class="fa-solid fa-money-bill"></i> {{__('Pay') }}</b></a> @if($allData[0]->total_amount > $totalPaid) <a href="#paymentModal" data-toggle="modal" class="btn btn-success btn-sm"> <i class="fa fa-money"></i><b> {{__('sale_milk.pay_due') }}</b></a> @endif </td>
          </tr>
          @endif
          </tbody>
