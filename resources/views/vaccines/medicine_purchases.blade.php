...
@section('content')
    ...
    <section class="content-header">
        <h1><i class="fa-solid fa-capsules"></i> {{ __('Medicine') }}</h1>
        <ol class="breadcrumb">
        ...
        <div class="box box-success">
            ...
            <div class="box-body">
                <div class="col-md-8 col-md-offset-2" style="margin-left:0px; width: 100%;">
                    <div class="panel panel-default">
                        <div class="panel-heading feed-heading feed-heading"><i
                                class="fa fa-info-circle"></i>&nbsp;{{ __('Search By Date') }} :</div>
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <form action="{{ route('medicine_purchases_by_date') }}" method="GET">
                                        @csrf
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="date">{{ __('cow_feed.date') }} <span
                                                        class="validate">*</span> : </label>
                                                <input type="date" name="fdate" class="form-control wsit_datepicker"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="date">{{ __('cow_feed.date') }} <span
                                                        class="validate">*</span> : </label>
                                                <input type="date" name="tdate" class="form-control wsit_datepicker"
                                                    value="{{ !empty($single_data->date) ? date('m/d/Y', strtotime($single_data->date)) : '' }}"
                                                    required>
                                            </div>
                                        </div>
                ...
                <table id="cowFoodTable" class="table table-bordered">
                    <button onclick="window.print()" class="btn btn-primary">Print <i class="fa-solid fa-print"></i>
                    </button>
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        @foreach ($BuyMedicine as $index => $BuyMedicines)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $BuyMedicines->supplier_name }}</td>
                                <td>
                                    @if($BuyMedicines->purpose_id == 14)
                                        {{ $BuyMedicines->medicine_name }}
                                    @elseif($BuyMedicines->purpose_id == 13)
                                        {{ $BuyMedicines->vaccine_name }}
                                    @else
                                        N/A
                                    @endif
                                </td>

                                <td>{{ $BuyMedicines->quantity }} {{ $BuyMedicines->unit }}</td>
                                <!--<td>{{ $BuyMedicines->price }}</td>-->
                                <td>{{ $BuyMedicines->description }}</td>
                                <td>{{ $BuyMedicines->date }}</td>
                                <td>
                                    <a href="{{ route('medicine_purchases.edit', $BuyMedicines->id) }}"
                                        class="btn btn-sm btn-primary">Edit</a>
                                    <form action="{{ route('medicine_purchases.destroy', $BuyMedicines->id) }}"
