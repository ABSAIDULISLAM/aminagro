...
<form action="{{ isset($medicine) ? route('medicine.update', $medicine->id) : route('medicine.store') }}" method="POST">
    @csrf
    @if(isset($medicine))
        @method('PUT')
    @endif
    ...
    <div class="form-row">
        <div class="form-group col-md-12">
            ...
            <input type="text" class="form-control" id="name" placeholder="Medicine name" 
                   name="name" value="{{ isset($medicine) ? $medicine->name : old('name') }}" required>
        </div>

        <div class="form-group col-md-12">
            <label for="description">Description</label>
            <textarea class="form-control" id="description" rows="3" 
                      name="description">{{ isset($medicine) ? $medicine->description : old('description') }}</textarea>
        </div>
    </div>
    
    <button type="submit" class="btn btn-primary" style="margin-left: 20px;">
        {{ isset($medicine) ? 'Update' : 'Save' }}
    </button>
    <a href="{{ route('medicine-list') }}" class="btn btn-secondary">Cancel</a>
</form>
    @if ($errors->any())
