...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{ $medicines->stock }} {{ $medicines->unit }}</td>
      <td>{{ $medicines->description }}</td>
        <td>
        <a href="{{ route('edit_medicine', $medicines->id) }}" class="btn btn-sm btn-primary">Edit</a>
        <form action="{{ route('delete_medicine', $medicines->id) }}" method="GET" style="display: inline;">
        @csrf
        @method('GET')
