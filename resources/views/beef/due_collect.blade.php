...
@section('title', __('Due Collect'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="icon-list"></i> {{__('Due Collect') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('Due Collect') }}</li>
  </ol>
</section>
      ...
      <div class="col-md-8 col-md-offset-2" style="margin-left:0px; width: 100%;">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <form action="{{ route('due_collect_by_date') }}" method="GET">
                     @csrf
                <div class="col-md-6">
...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    @foreach($sellbeef as $index => $sellbeefs)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $sellbeefs->buyer_name }}</td>
      <td>{{ $sellbeefs->qty }}</td>
      <td>{{ $sellbeefs->price }}</td>
      <td>{{ $sellbeefs->total_price }}</td>
      <td>{{ $sellbeefs->due }}</td>
      <td>{{ $sellbeefs->branch_name }}</td>
      <td>{{ $sellbeefs->date }}</td>
      <td><a href="{{ route('sell_beef.edit', $sellbeefs->id) }}" class="btn btn-warning btn-sm">
    <i class="fa-solid fa-pen-to-square"></i> Edit
</a> <form action="{{ route('sell_beef.destroy', $sellbeefs->id) }}" method="POST" style="display:inline;">
    @csrf
    @method('DELETE')
...
<a href="{{ route('sell_beef.pay', $sellbeefs->id) }}" class="btn btn-success btn-sm">
    <i class="fa-solid fa-money-bill"></i></i> pay
</a>
</td>
    </tr>
    @endforeach
