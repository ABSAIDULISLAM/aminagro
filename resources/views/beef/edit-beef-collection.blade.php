...
<form action="{{ route('beef_collection.update', $beef->id) }}" method="POST">
    @csrf


    <div class="form-group col-md-12">
      <label for="inputEmail4">তারিখ*</label>
      <input type="date" class="form-control" id="inputEmail4" placeholder="তারিখ" name="date" value="{{ $beef->date }}" style="width: 25%;">
    </div>
<div class="form-group col-md-12">
    ...
    <select class="form-control select2" id="exampleFormControlSelect1" name="cow">
        @foreach($animal as $animals)
            <option value="{{ $animals->id }}" {{ $beef->cow == $animals->id ? 'selected' : '' }}>
            {{ $animals->id }}
            </option>
        @endforeach
    </select>
...
<script>
    ...
    <div class="form-group col-md-12">
      <label for="inputPassword4">মোট গোশত*</label>
      <input type="text" class="form-control" id="inputPassword4" placeholder="মোট গোশত*"name="qty" value="{{ $beef->qty }}">   
    </div>

    <div class="form-group col-md-12">
    <label for="exampleFormControlTextarea1">বিবরণ*</label>
    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3"name="description"> {{ $beef->details }} </textarea>
    </div>
