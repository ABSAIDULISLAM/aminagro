      ...
      <div class="col-md-8 col-md-offset-2" style="margin-left:0px; width: 100%;">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <form action="{{ route('beef-in-stock-by-date') }}" method="GET">
                     @csrf
                <div class="col-md-6">
...
<table id="cowFoodTable" class="table table-bordered">
  ...
  <tbody>
    @foreach($BeefCollection as $index => $BeefCollections)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $BeefCollections->cow }}</td>
      <td>{{ $BeefCollections->qty }} {{ $BeefCollections->unit }}</td>
      <td>{{ $BeefCollections->details }}</td>
      <td>{{ $BeefCollections->date }}</td>
      <td>        <a href="{{ route('beef_collection.edit', $BeefCollections->id) }}" class="btn btn-warning btn-sm">
          <i class="fa-solid fa-pen-to-square"></i> Edit
        </a>         
        <form action="{{ route('beef_collection.destroy', $BeefCollections->id) }}" method="POST" style="display:inline;">
            @csrf
            @method('DELETE')
