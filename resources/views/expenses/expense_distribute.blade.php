...
@section('content')
    ...
    <section class="content-header">
        <h1><i class="icon-list"></i> {{ __('খরচ বন্টন') }}</h1>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> {{ __('same.home') }}</a></li>
            <li class="active">{{ __('খরচ বন্টন') }}</li>
        </ol>
    </section>
    <section class="content">
        ...
        <div class="box box-success">
            <div style="padding: 50px;">
                <form action="{{ route('save-expense-distribute') }}" method="POST">
                    @csrf
                    ...
                    {{-- <div class="form-group col-md-12">
                        ...
                        <select class="form-control selectpicker" id="exampleFormControlSelect1" name="cow[]" multiple data-live-search="true" title="Select options..." required>
                            @foreach ($animal as $animals)
                                <option value="{{ $animals->id }}" {{ in_array($animals->id, old('cow', [])) ? 'selected' : '' }}>
                                    {{ $animals->id }}
                                </option>
                            @endforeach
                        </select>
