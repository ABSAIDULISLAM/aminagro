...
@section('title', __('cow medicine monitor'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="fa fa-eyedropper"></i> {{__('cow medicine monitor') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('cow medicine monitor') }}</li>
  </ol>
</section> 
<section class="content">
  ...
  <div class="box box-success"> @if(empty($single_data))
    {{  Form::open(array('route' => 'medicine-monitor-store', 'method' => 'post', 'files' => true))  }}
    ...
    <div class="box-body">
      ...
      <div class="col-md-10 col-md-offset-1">
        <div class="panel panel-default">
          <div class="panel-heading feed-heading"><i class="fa fa-edit"></i>&nbsp;{{__('Medicine Date & Note') }} :</div>
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <div class="col-md-12">
      ...
      <div class="col-md-10 col-md-offset-1">
        <div class="panel panel-default">
          <div class="panel-heading feed-heading"><i class="icon-list"></i>&nbsp;{{__('Medicine List') }} :</div>
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <table class="table table-responsive table-bordered table-striped">
                  ...
                  @if(isset($single_data))
                  <?php  
						$checkData = DB::table('cow_medicine_monitor_dtls')
									 ->where('monitor_id', $single_data->id)
									 ->where('vaccine_id', $vaccine_data->id)
									 ->exists();
						if($checkData==true){
							$savedData = DB::table('cow_medicine_monitor_dtls')
									 ->where('monitor_id', $single_data->id)
									 ->where('vaccine_id', $vaccine_data->id)
									 ->first();
						}
                  ...
                  <tr>
                    <td><label class="checkbox-inline">
                      ...
                      {{$vaccine_data->name}} - ( {{$vaccine_data->months}} {{__('cow_vaccine_monitor.days') }} ) </label>
                    </td>
                    ...
                    <td><input type="text" name="cow_vaccine[{{$row}}][qty]" class="form-control" value="{{(isset($checkData)?($checkData==true)?$savedData->remarks:'':'')}}">
                    </td>
                    <td><input type="time" name="cow_vaccine[{{$row}}][time]" class="form-control" value="{{(isset($checkData)?($checkData==true)?$savedData->time:'':'')}}">
                    </td>
                  </tr>
                  <?php $row++; ?>
                  @endforeach
