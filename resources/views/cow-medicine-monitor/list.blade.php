...
<section class="content-header">
  <h1><i class="icon-list"></i> {{__('Medicine Details') }}</h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
    <li class="active">{{__('Medicine Details') }}</li>
  </ol>
</section>
<section class="content"> @include('common.message')
  <div class="box box-success">
    <div class="box-header with-border" align="right"> <a href="{{  url('medicine-monitor-create')  }}" class="btn btn-success btn-sm"> <i class="fa fa-plus-square"></i> <b>{{__('same.add_new') }}</b> </a> <a href="{{URL::to('vaccine-monitor')}}" class="btn btn-warning btn-sm"> <i class="fa fa-refresh"></i> <b>{{__('same.refresh') }}</b> </a> </div>
    <div class="box-body">
      <div class="row">
        <div class="col-sm-12">
                      ...
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <div class="modal-header">
                            ...
                            <h4 class="modal-title"><i class="fa fa-info-circle edit-color"></i> Medicine Details</h4>
                          </div>
                          <div class="modal-body">
                            <table class="table table-bordered table-striped table-responsive">
                              ...
                              <tbody>
                                <?php  
                                    $dtlsArr = DB::table('cow_medicine_monitor_dtls')
                                            ->leftJoin('medicines', 'medicines.id','cow_medicine_monitor_dtls.vaccine_id')
                                            ->where('cow_medicine_monitor_dtls.monitor_id', $data->id)
                                            ->select('medicines.name', 'cow_medicine_monitor_dtls.*')
                                            ->get();
                                  ?>
                              @foreach($dtlsArr as $dtlsArrData)
                              <tr>
                                <td>{{$dtlsArrData->name}}</td>
