...
@section('title', __('Fish Sale Invoice'))
@section('content')
<section class="content-header">
  <h1><i class="icon-docs"></i> {{__('Fish Sale Invoice') }}</h1>
  <ol class="breadcrumb">
    <li><i class="fa fa-dashboard"></i> {{__('same.home') }}</li>
    <li class="active">{{__('Fish Sale Invoice') }}</li>
  </ol>
</section>
<?php 
  ...
  <div class="box box-success">
    ...
    <div class="box-body">
      <div class="container" id="print-box">
        <div class="card">
          <div class="card-header"> {{__('Fish Sale Invoice') }} <strong> #000{{$single_data->id}}</strong> <span class="float-right"> <strong>{{__('sale_milk.issue_date') }} :</strong> {{Carbon\Carbon::parse($single_data->date)->format('d/m/Y')}}</span> </div>
          <div class="card-body">
            <div class="row mb-4">
              <div class="col-sm-8">
              ...
              <table class="table table-striped invoice_table">
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="left">{{__('Fish Sale') }}</td>
                    <td class="center">{{$single_data->qty}} {{$single_data->unit}}</td>
                    <td class="right">{{App\Library\farm::currency($single_data->price)}}</td>
                    <td class="right">{{App\Library\farm::currency($single_data->total_price )}}</td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr>
