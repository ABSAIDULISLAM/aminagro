...
@section('title', __('Sick Animal'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="fa-solid fa-cow"></i> {{__('Sick Animal') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('Sick Animal') }}</li>
  </ol>
</section>
...
<a href="{{URL::to('add-sick-cow')}}" class="btn btn-success btn-sm" data-toggle="modal"> <i class="fa fa-plus-square"></i> <b>Add New</b> </a> &nbsp;
    ...
    <tr>
      ...
      <td>{{ $DeadAnimals->date }}</td>
       <td>
        ...
        <form action="{{ route('sick-cow.destroy', $DeadAnimals->id) }}" method="POST" style="display: inline;">
          @csrf
          @method('DELETE')
