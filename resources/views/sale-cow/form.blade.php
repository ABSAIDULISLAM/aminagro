		    	...
		    	<div class="panel-body">
		    		<div class="row">
		    			<div class="col-md-12"> 
		    				<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="date">{{__('cow_sale.date') }} <span class="validate">*</span> : </label>
                                        <input type="date" name="sale_date" class="form-control wsit_datepicker" 
		    				...
		    				<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="customer_name">{{__('cow_sale.customer_name') }} <span class="validate">*</span> : </label>
                                        <select class="form-control select2" id="exampleFormControlSelect1" name="customer_name">
                                        @foreach($buyer as $buyers)
                                        <option value="{{ $buyers->id }}">{{ $buyers->name }} - Balance : {{ $buyers->balance }}</option>
                                        @endforeach
                                        </select>
							   	</div>
                                <script>
							...
							<div class="col-md-4"> 
								<div class="form-group">
							    	<label for="customer_number">{{__('cow_sale.customer_phone') }} : </label>
							    	<input type="text" name="customer_number" class="form-control" value="{{(!empty($single_data->customer_number))?$single_data->customer_number:''}}">
							   	</div>
							</div>
							<!--<div class="col-md-12"> -->
							<!--	<div class="form-group">-->
							<!--    	<label for="customer_number">{{__('cow_sale.customer_email') }} : </label>-->
							<!--    	<input type="text" name="email" class="form-control" value="{{(!empty($single_data->email))?$single_data->email:''}}" >-->
							<!--   	</div>-->
							<!--</div>-->
							<div class="col-md-12"> 
								<div class="form-group">
		    	...
		    	<div class="panel-body">
		    		<div class="row">
		    			<div class="col-md-12"> 
							...
							<div class="col-md-3"> 
								<div class="form-group">
							    	<label for="user_balance">{{__('Take an amount from user balance') }} : </label>
							    	<input type="text" name="user_balance" class="form-control" value="{{(!empty($single_data->total_paid))?$single_data->total_paid:''}}" id="user_balance" onkeyup="calculateCowSell();" required>
							   	</div>
							</div>
						</div>
					</div>
...
document.addEventListener("DOMContentLoaded", function () {
    function calculateCowSell() {
        var total_price = 0, total_paid = 0, user_balance = 0, due = 0;

        if (document.getElementById("total_price").value) {
            total_price = parseFloat(document.getElementById("total_price").value) || 0;
        }
        if (document.getElementById("total_paid").value) {
            total_paid = parseFloat(document.getElementById("total_paid").value) || 0;
        }
        if (document.getElementById("user_balance").value) {
            user_balance = parseFloat(document.getElementById("user_balance").value) || 0;
        }

        // Calculate due amount: total_price - total_paid - user_balance
        due = total_price - total_paid - user_balance;
    ...
    function totalPriceCowSell() {
        var total = 0;
        var cowPrices = document.querySelectorAll(".cowprice");

        cowPrices.forEach(function (input) {
            var value = input.value !== '' ? parseFloat(input.value) : 0;
            total += value;
        });
    ...
    document.getElementById("total_price").addEventListener("keyup", calculateCowSell);
    document.getElementById("total_paid").addEventListener("keyup", calculateCowSell);
    document.getElementById("user_balance").addEventListener("keyup", calculateCowSell);
});
</script>
