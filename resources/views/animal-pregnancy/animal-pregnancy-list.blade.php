...
@section('title', __('Animal Pregnancy List'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="fa-solid fa-cow"></i> {{__('Animal Pregnancy List') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('Animal Pregnancy List') }}</li>
  </ol>
</section>
...
<button onclick="window.print()" class="btn btn-primary">Print <i class="fa-solid fa-print"></i> </button>
  ...
  <tbody>
    @foreach($pregnancylist as $index => $pregnancylists)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $pregnancylists->shed_name }}</td>
      <td>{{ $pregnancylists->cow_id }}</td>
      <td>{{ $pregnancylists->pregnancy_type_name }}</td>
      <td>{{ $pregnancylists->animal_type_name }}</td>
      <td>{{ $pregnancylists->semen_push_date }}</td>
      <td>{{ $pregnancylists->pregnancy_start_date }}</td>
      <td>{{ $pregnancylists->semen_cost }}</td>
      <td>{{ $pregnancylists->other_cost }}</td>
      <td>{{ $pregnancylists->note }}</td>
    </tr>
    @endforeach
  </tbody>
</table>
