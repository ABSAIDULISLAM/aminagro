            ...
            <div class="form-group">
              ...
              <select class="form-control" name="employee_id" id="employeeList" required>
                <option value="">{{__('same.select') }}</option>
                  ...
                  @else
                  	@foreach($allEmployee as $employee)
                		<option value="{{$employee->id}}" data-salary="{{$employee->gross_salary}}"> {{$employee->name}} ( ID #{{$employee->id}} ) - Advance Payment : {{$employee->advance_payment}} </option>
                    @endforeach
                  @endif
              </select>
            </div>
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <input type="text" name="salary" id="salary" value="{{isset($employeeSalary->salary)?$employeeSalary->salary:old('salary')  }}" class="form-control decimal" placeholder="Salary" required>
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
              ...
              <div class="col-md-3">
                <div class="form-group">
                  <label for="addition_money">{{__('Advance Money') }} :</label>
                  <input type="text" name="advance" id="advance_money" value="{{isset($employeeSalary->addition_money)?$employeeSalary->advance_money:old('advance_money')  }}" class="form-control decimal" placeholder="Advance Money">
                </div>
              </div>
             <div class="col-md-3">
                <div class="form-group">
                  <label for="addition_money">{{__('Pay from advance') }} :</label>
                  <input type="text" name="Pay_from_advance_payment" id="Pay_from_advance_payment" class="form-control decimal" placeholder="Pay from advance payment">
                </div>
              </div>
            </div>
            <div class="form-group">
