...
@section('title', __('এডভান্স স্টাফ বেতন তালিকা'))
@section('content')
<!-- Content Header (Page header) -->
<section class="content-header">
  <h1><i class="fa-solid fa-money-bill-transfer"></i> {{__('এডভান্স স্টাফ বেতন তালিকা') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{URL::to('dashboard')}}"><i class="fa fa-dashboard"></i> {{__('same.home') }}</a></li>
	<li class="active">{{__('এডভান্স স্টাফ বেতন তালিকা') }}</li>
  </ol>
</section>
...
<button onclick="window.print()" class="btn btn-primary">Print <i class="fa-solid fa-print"></i> </button>
  ...
  <tbody>
    @foreach($AdvanceSalary as $index => $AdvanceSalarys)
    <tr>
      <td>{{ $index + 1 }}</td>
      <td>{{ $AdvanceSalarys->user_name }}</td>
      <td>{{ $AdvanceSalarys->advance }}</td>
      <td>{{ $AdvanceSalarys->note }}</td>
      <td>{{ $AdvanceSalarys->date }}</td>
    </tr>
    @endforeach
  </tbody>
</table>
