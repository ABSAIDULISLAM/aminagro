...
<div class="row ajax-page-pt15">
  <div class="col-md-12">
    <div class="col-md-6">
    @if(isset($cow_score->new_images) && !empty($cow_score->new_images))
    ...
    <div class="col-md-6 border paymentsttausbox">
      <table class="table table-bordered table-striped table-responsive">
        ...
        <tr>
          <td><b>{{__('ক্রয় মূল্য') }}</b></td>
          <td>{{$single_data->buying_price ??''}}&#2547;</td>
        </tr>
        <tr>
          <td><b>{{__('Sell Price') }}</b></td>
          <td>{{$cow_sell_price->price ??''}}&#2547;</td>
        </tr>
        <tr>
...
<div class="row ajax-page-pt15" style="display:none;">
  <div class="col-md-12">
    <table class="table table-bordered table-striped table-responsive">
      <thead>
        <tr>
...
@php
    ...
    if (!$animal) {
        $animal = DB::table('calf')
                  ->leftJoin('animal_type', 'animal_type.id', '=', 'calf.animal_type')
            ->where('calf.id', $animalId)
                ->select('calf.*', 'animal_type.type_name')
            ->first();
    }

    // Fetch all vaccine records for this animal
    $vaccineRecords = DB::table('cow_vaccine_monitor')
        ->where('cow_id', $animalId)
        ->orderBy('date', 'desc')
        ->get(    );

    // Prepare vaccine details and calculate total price
    $totalPrice = 0;
    $vaccineData = [];

    foreach ($vaccineRecords as $record) {
        $details = DB::table('cow_vaccine_monitor_dtls')
            ->join('vaccines', 'vaccines.id', '=', 'cow_vaccine_monitor_dtls.vaccine_id')
            ->where('monitor_id', $record->id)
            ->select(
                'cow_vaccine_monitor_dtls.*',
                'vaccines.vaccine_name',
                'vaccines.months',
                'vaccines.repeat_vaccine',
                'vaccines.dose',
                'vaccines.price',
                'vaccines.note as vaccine_note'
            )
            ->get();

        foreach ($details as $detail) {
            $totalPrice += $detail->price ?? '0';
            $vaccineData[$record->date][] = $detail;
        }
    }
@endphp
...
<div class="row ajax-page-pt15">
  <div class="col-md-12">
    <table class="table table-bordered table-striped table-responsive">
      ...
      <tbody>
        <tr>
          {{-- <!--<td>{{ $animal->stall_no ?? 'N/A' }}</td>--> --}}
          <td>{{ $animal->type_name ?? 'N/A' }}</td>
          <td>{{ $animal->gender ?? 'N/A' }}</td>
          <td>
            @if(isset($animal->DOB))
              {{ date('m/d/Y', strtotime($animal->DOB)) }}
            @else
              N/A
            @endif
          </td>
          <td>
            @if(isset($animal->DOB))
              @php
                $birthDate = new DateTime($animal->DOB);
                $today = new DateTime();
          ...
          <td>{{ $animal->height ?? 'N/A' }}</td>
          <td>{{ $animal->status ?? 'N/A' }}</td>
        </tr>
      </tbody>
    </table>
  </div>
    ...
    @if(count($vaccineData) > 0)
      <table class="table table-bordered table-striped table-responsive">
        ...
        <tbody>
          ...
          @foreach($vaccineData as $date => $vaccines)
            @foreach($vaccines as $vaccine)
              <tr>
                <td>{{ $counter++ }}</td>
                <td>{{ $vaccine->vaccine_name ?? ''}}</td>
                <td>{{ date('m/d/Y', strtotime($date)) }}</td>
                <td>{{ $vaccine->months }} months</td>
                <td>{{ $vaccine->repeat_vaccine ? 'Yes' : 'No' }}</td>
                <td>{{ $vaccine->dose ?? '' }}</td>
        ...
        <tfoot>
          <tr>
            <td colspan="6" class="text-right"><strong>Total Vaccine Cost:</strong></td>
            <td colspan="2"><strong>{{ number_format($totalPrice, 2) }}</strong></td>
          </tr>
        </tfoot>
      </table>
    @else
      <div class="alert alert-info">No vaccination records found for this animal.</div>
    @endif
  </div>
</div>
...
@php
    ...
    // Fetch medicine monitor entries for this animal
    $medicineMonitors = DB::table('cow_medicine_monitor')
        ->where('cow_id', $animalId)
        ->orderBy('date', 'desc')
        ->get();
    ...
    foreach ($medicineMonitors as $monitor) {
        $details = DB::table('cow_medicine_monitor_dtls')
            ->join('medicines', 'medicines.id', '=', 'cow_medicine_monitor_dtls.vaccine_id') // actually medicine_id
            ->where('monitor_id', $monitor->id)
            ->select(
                'cow_medicine_monitor_dtls.*',
        ...
        foreach ($details as $detail) {
            $totalMedicineCost += $detail->price ?? '0';
            $medicineData[$monitor->date][] = $detail;
        }
    }
@endphp
    ...
    <div class="col-md-12">
        ...
        @if(count($medicineData) > 0)
            <table class="table table-bordered table-striped table-responsive">
                ...
                <tbody>
                    @php $counter = 1; @endphp
                    @foreach($medicineData as $date => $medicines)
                ...
                <tfoot>
                    <tr>
                        <td colspan="6" class="text-right"><strong>Total Medicine Cost:</strong></td>
                        <td colspan="2"><strong>{{ number_format($totalMedicineCost, 2) }}</strong></td>
                    </tr>
                </tfoot>
            </table>
        @else
            <div class="alert alert-info">No medicine records found for this animal.</div>
        @endif
    </div>
</div>
...
@php
    ...
    // Fetch all cow_feed records for the animal
    $feedRecords = DB::table('cow_feed')
        ->where('cow_id', $animalId)
        ->orderBy('date', 'desc')
        ->get();
    ...
    foreach ($feedRecords as $record) {
        $details = DB::table('cow_feed_dtls')
            ->join('food_items', 'food_items.id', '=', 'cow_feed_dtls.item_id')
            ->join('food_units', 'food_units.id', '=', 'cow_feed_dtls.unit_id')
            ->leftJoin('cow_food', function($join) {
                $join->on('cow_food.food', '=', 'cow_feed_dtls.item_id');
            })
            ->where('cow_feed_dtls.feed_id', $record->id)
            ->select(
                'cow_feed_dtls.*',
                'food_items.name as food_name',
                'food_units.name as unit_name',
                'cow_food.price as price_per_unit',
                DB::raw('cow_feed_dtls.qty * IFNULL(cow_food.price, 0) as total_price')
            )
            ->get();

        foreach ($details as $detail) {
            $grandTotal += $detail->total_price ?? 0;
            $feedData[$record->date][] = $detail;
        }
    }
@endphp
  ...
  <div class="col-md-12">
    @if(count($feedData) > 0)
      <table class="table table-bordered table-striped table-responsive">
        ...
        <tbody>
          @php $counter = 1; @endphp
          @foreach($feedData as $date => $items)
            @foreach($items as $item)
              <tr>
                ...
                <td>{{ number_format($item->price_per_unit ?? 0, 2) }}</td>
        ...
        <tfoot>
          <tr>
            <td colspan="6" class="text-right"><strong>Total Feed Cost:</strong></td>
            <td colspan="2"><strong>{{ number_format($grandTotal, 2) }}</strong></td>
          </tr>
        </tfoot>
      </table>
    @else
      <div class="alert alert-info">No feed records found for this animal.</div>
    @endif
  </div>
</div>
...
<div class="row">
  <div class="col-md-12 ajax-page-bb" align="center">
    <h3><b>Permanent Expense List</b></h3>
  </div>
</div>
  ...
  <div class="col-md-12">
    @if(count($additional_expense) > 0)
      <table class="table table-bordered table-striped table-responsive">
        ...
        <tbody>
          @foreach($additional_expense as $index => $expense)
            <tr>
              <td>{{ $index + 1 }}</td>
              <td>{{ $expense->name ?? 'N/A' }}</td>
              <td>{{ \Carbon\Carbon::parse($expense->date)->format('m/d/Y') }}</td>
              <td>{{ $expense->description ?? 'N/A' }}</td>
              <td>{{ number_format($expense->price ?? 0, 2) }}</td>
            </tr>
          @endforeach
        </tbody>
        <tfoot>
          <tr>
            ...
            <td><strong>{{ number_format($additional_expense_sum, 2) }}</strong></td>
          </tr>
        </tfoot>
      </table>
    @else
      <div class="alert alert-info">No permanent expense records found for this animal.</div>
    @endif
  </div>
</div>
...
<div class="row">
  <div class="col-md-12 ajax-page-bb" align="center">
    <h3><b>Milk Collection List</b></h3>
  </div>
</div>
  ...
  <div class="col-md-12">
    @if(count($additional_expense) > 0)
      <table class="table table-bordered table-striped table-responsive">
        ...
        <tbody>
          @foreach($milk_collection as $index => $expense)
            <tr>
              <td>{{ $index + 1 }}</td>
              <td>{{ $expense->date ?? 'N/A' }}</td>
              <td>{{ number_format($expense->total ?? 0, 2) }}</td>
            </tr>
          @endforeach
        </tbody>
        <tfoot>
          <tr>
            ...
            <td><strong>{{ number_format($milk_collection_sum, 2) }}</strong></td>
          </tr>
        </tfoot>
      </table>
    @else
...
<!-- Page Title -->
<div class="row">
  <div class="col-md-12 ajax-page-bb text-center">
  ...
  <div class="col-md-12">
    <div class="table-responsive">
      <table class="table table-bordered table-striped">
        ...
        <tbody>
          @foreach($food as $index => $item)
            <tr>
              <td>{{ $index + 1 }}</td>
              <td>{{ $item->food_name }}</td>
              <td>{{ $item->cow_feed_dtls_qty }} {{ $item->cow_feed_dtls_unit }}</td>
              <td>{{ \Carbon\Carbon::parse($item->feeding_date)->format('d/m/Y') }}</td>
              <td>
        ...
        <tfoot>
          <tr>
            <td colspan="5" class="text-right"><strong>Total Feed Cost:</strong></td>
            <td><strong>{{ number_format($totalPriceAllRecords, 2) }}</strong></td>
          </tr>
        </tfoot>
      </table>
    </div>
...
<div style="font-weight: bold; font-size: 1.2em; margin-top: 20px;">
    {{-- <p>&nbsp;Total Profit: {{ number_format($cow_sell_price->price + $milk_collection_sum - $totalPriceAllRecords - $additional_expense_sum - $totalMedicineCost - $totalPrice - $single_data->buying_price, 2) }}</p> --}}

    @php
    $sellPrice = isset($cow_sell_price->price) ? $cow_sell_price->price : 0;
    $buyingPrice = isset($single_data->buying_price) ? $single_data->buying_price : 0;
    $medicineCost = isset($totalMedicineCost) ? $totalMedicineCost : 0;
    $vaccineCost = isset($totalPrice) ? $totalPrice : 0;

    $profit = $sellPrice
            + $milk_collection_sum
            - $totalPriceAllRecords
            - $additional_expense_sum
            - $medicineCost
            - $vaccineCost
            - $buyingPrice;
@endphp
