...
<section class="content-header">
  <h1><i class="fa-solid fa-cow"></i> {{__('Spoiled Milk') }}</h1>
  <ol class="breadcrumb">
      ...
      <div class="col-md-8 col-md-offset-2" style="margin-left:0px; width: 100%;">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <div class="row">
              <div class="col-md-12">
                <form action="{{ route('spoiled_milk_by_date') }}" method="GET">
                     @csrf
                <div class="col-md-6">
...
<table id="cowFoodTable" class="table table-bordered">
    ...
    <tbody>
        @foreach($SpoiledMilk as $index => $SpoiledMilks)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>{{ $SpoiledMilks->account }}</td>
            <td>{{ $SpoiledMilks->quantity }}</td>
            <td>{{ $SpoiledMilks->price }}</td>
            <td>{{ $SpoiledMilks->description }}</td>
            <td>{{ $SpoiledMilks->date }}</td>
            <td>
                <!-- Edit Button -->
                <a href="{{ route('spoiled_milk.edit', $SpoiledMilks->id) }}" class="btn btn-sm btn-primary">
                    <i class="fa-solid fa-pen-to-square"></i> Edit
                </a>
                
                <!-- Delete Button with confirmation -->
                <form action="{{ route('spoiled_milk.destroy', $SpoiledMilks->id) }}" method="POST" style="display: inline;">
                    @csrf
                    @method('DELETE')
